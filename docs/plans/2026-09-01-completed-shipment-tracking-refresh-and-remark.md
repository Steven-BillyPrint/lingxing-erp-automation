...
读取近 15 天本系统自动完成任务
  ...
  -> 收到唯一“全部操作成功”回执后完成；不再强制读取已移出“可更新”的原行
  -> 完成周期并保留旧值、新值和全过程证据
```
...
每个外部写动作之前，必须先在 SQLite 事务中记录 intent、目标单号、payload hash 和当前权威状态；动作后必须通过独立读回或该操作唯一、明确的成功结果弹窗推进检查点。不能仅根据“点击成功”、HTTP 200 或普通页面 toast 判定业务成功。
...
7. 确认唯一“全部操作成功”回执并尝试点击“我知道了”；关闭结果弹窗只是页面清理，不影响已经收到的成功回执。

读回规则：

- 提交前必须从唯一目标行读回“系统标发单号”包含本周期新运单号；提交后收到唯一“全部操作成功”回执才推进 `MARK_UI_CONFIRMED`；
- “可更新”是瞬时队列，成功后目标行可能立即移出；不得在成功回执之后强制从原表读取该行，也不轮询“标发中/已完成”；
- 已记录提交意图但没有唯一成功弹窗时，不自动重复点击，转人工核对；
...
10. “可更新”已包含新系统标发单号时准确标发；唯一“全部操作成功”回执出现后完成，即使目标行已从原表移出也不误报失败。
11. 标发点击后断线且没有收到唯一成功回执：转人工，不重复点击。
...
- 提交前目标行“系统标发单号”包含新单号，提交后收到唯一“全部操作成功”回执；允许目标行随即移出“可更新”；
...
- 提交前“系统标发单号”列已读到 `OnTrac ： 1LSD01R0018AGMD`；提交后唯一“全部操作成功”结果弹窗确认领星已接受更新，本地周期随即完成，不再依赖可能已移出“可更新”的原行。
