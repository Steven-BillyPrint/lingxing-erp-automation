...
def _notification_matches_expected(
    ...
    placeholders = tuple(
        f"Package {stable_package_label(expected.package_complete + offset + 1)}: "
        "Available soon."
        for offset in range(expected.package_missing)
    )
    if body.count("Available soon") != expected.package_missing or any(
        placeholder not in body for placeholder in placeholders
    ):
    ...
    if str(notification.get("channel") or "") in {CHANNEL_EMAIL, CHANNEL_MANUAL_EMAIL}:
        ...
        if (
            ...
            or any(placeholder not in body_html for placeholder in placeholders)
        ):
...
def _notification_summary(notification: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "placeholders": [
            f"Package {stable_package_label(package_complete + offset + 1)}: "
            "Available soon."
            for offset in range(max(0, package_total - package_complete))
        ],
    }
