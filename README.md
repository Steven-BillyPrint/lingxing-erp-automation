...
当前版本采用“官方 API 能做的全部走 API，官方没有 API 的步骤才使用网页”的固定策略。旧脚本不会与新程序并行运行，发布时仅保存在经过最终业务规则验收的 Git 回退分支中。
...
- “定制订单”：API 扫描 Amazon 候选，处理选中订单，查看或修改阶段状态，并可从指定阶段重新打开工作流。
- “自动标发”：API 扫描当前“待审核”整表内全部平台、最近 30 个自然日购买时间范围的所有分页，不检查付款时间；可以手动添加订单，查看队列任务状态，查询阿里国际物流，执行选中标发，以及受控地重试、取消/恢复、人工完成/撤销任务。
- “状态管理”：查看后台任务、能力实际模式和 ERP 写入急停开关。
- “设置”：统一编辑加密配置、测试 API、迁移旧状态和跨电脑迁移。
- “日志”：无需额外账号或权限即可查看简洁的任务结果；可以按任务 ID 查看完整扫描审计文件，或直接打开该日志所在目录。
...
- 自动标发显示购买时间查询范围、API 读取订单数、可判断订单数、标签命中数、候选数、本次新增数、重复数、刷新数、人工检查数、标签移除后自动暂停数、标签恢复数、立即重试数、邮件预览补建数，以及当前队列任务总数。
...
- **自动标发**扫描领星当前“待审核”整表：不限制平台，按中国时区最近 30 个自然日的购买时间读取全部分页，只要订单带有“帐篷标发”标签就进入候选判断，完全不读取或校验付款时间。API 查询按允许的最大区间拆窗，边界重叠的数据按领星系统订单 ID 去重。
...
“自动标发”页面还允许用户按订单身份手动添加队列任务，并显示物流、ERP、邮件预览等阶段的当前状态。界面只提供受控状态变更：重试、取消、恢复、人工完成和撤销人工完成；每次操作都记录操作者、原因、前后状态和时间。人工完成只改变本地工作流判定，不能伪造领星 ERP 已写入，也不会跳过真正执行 ERP 标发时的急停检查和写入确认。
...
- 邮件永远是 `preview_only`：只生成本地批次和预览，不连接邮箱、不真实发送，也不受定制订单的 96 小时付款窗口限制。每次自动标发扫描成功后会为整个队列补建符合条件但尚未生成的预览；重复扫描不会生成重复批次。
- 日志页面不增加应用内访问权限，当前电脑用户可直接查看简洁的状态、时间、任务 ID、结果摘要和安全错误编号；文件仍受 Windows 自身的文件权限保护。
- 每次 API 定制或标发扫描都会生成一个与桌面任务 ID 一一对应的完整审计文件：

  ```text
  logs/api_scan/YYYY-MM-DD/<task_id>.json
  ```

  日志页可以从任务摘要打开该 JSON 或其所在目录。文件记录 schema/version、扫描类型、起止时间、安全查询摘要、分页轨迹、逐单决定、数量原值及规范化结果、汇总统计和 `error_id`；不会保存原始 API response 正文。
...
- 日志强制脱敏且不能关闭，不记录 AppSecret、Token、账号密码、完整邮箱、电话、收货地址或原始响应正文。任务 ID 只允许安全文件名字符，日志路径拒绝目录穿越、符号链接和 Junction 穿越。
...
该固定提交已经包含并验证三项最终业务规则：定制订单付款窗口为 96 小时、3x6m 帐篷拖轮包为 `x2`、整行替换数量等于原商品行初始数量。旧的 `codex/script-baseline-20260714` 仍是 24 小时语义且缺少最终整行数量规则，禁止直接用于本次发布回退。
