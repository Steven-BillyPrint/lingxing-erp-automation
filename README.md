...
- “自动标发”：API 扫描所有平台的候选；可以手动添加订单，查看队列任务状态，查询阿里国际物流，执行选中标发，以及受控地重试、取消/恢复、人工完成/撤销任务。
...
订单扫描采用失败关闭策略。只要分页未完整结束、总数不稳定、缺少关键字段或 API 返回失败，就不会把快照声明为完整：定制扫描不会写入候选数据库，也不会返回可操作订单；自动标发不会把不完整快照中的候选写入队列，也不会据此判定旧任务已经完成。当前已读取的安全统计和逐单判断仍会写入扫描审计，供排查使用。

扫描结果不会只显示一个容易误解的“新增数”：

- 定制扫描显示 API 读取订单数、规范化行数、候选数和各项 `skip_counts`。
- 自动标发显示 API 读取订单数、96 小时窗口内行数、标签命中数、候选数、本次新增数、重复数、刷新数、人工检查数，以及当前队列任务总数。
...
定制扫描只覆盖 Amazon；自动标发扫描不限制平台，会覆盖满足规则的所有平台订单。
...
该固定提交已经包含并验证三项最终业务规则：付款窗口为 96 小时、3x6m 帐篷拖轮包为 `x2`、整行替换数量等于原商品行初始数量。旧的 `codex/script-baseline-20260714` 仍是 24 小时语义且缺少最终整行数量规则，禁止直接用于本次发布回退。
